...
from strhub.data.custom_generator import Generator
...
warnings.filterwarnings(
    "ignore", category=UserWarning, module="fontTools.ttLib.tables._h_e_a_d"
)
...
def get_swa_lr_factor(
    warmup_pct, swa_epoch_start, div_factor=25, final_div_factor=1e4
) -> float:
...
def main(config: DictConfig):
    ...
    with open_dict(config):
        ...
        if gpu:
            ...
            config.trainer.precision = (
                "bf16-mixed"
                if torch.get_autocast_gpu_dtype() is torch.bfloat16
                else "16-mixed"
            )
    ...
    generators: Dict[str, Generator] = hydra.utils.instantiate(config.generators)
    datamodule.set_generators(**generators)
    ...
    swa_lr = config.model.lr * get_swa_lr_factor(
        config.model.warmup_pct, swa_epoch_start
    )
