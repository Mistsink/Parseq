...
class CharsetAdapter:
    ...
    def __init__(self, charset_path: str) -> None:
        super().__init__()
        with open(charset_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
            lines = [line.strip() for line in lines if line.strip()]
        target_charset = "".join(lines)
...
class BaseTokenizer(ABC):
    ...
    def __init__(
        ...
        specials_last: tuple = (),
        support_space: bool = True,
        support_line_break: bool = True,
    ) -> None:
        with open(charset_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        ...
        self.support_space = support_space
        if support_space:
            specials.append(self.SPACE_TOKEN)
        self.support_line_break = support_line_break
        if support_line_break:
            specials.append(self.LINE_BREAK_TOKEN)

        self._itos = tuple(charset) + specials_first + tuple(specials) + specials_last
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        for c in text:
            if c == " ":
                if self.support_space:
                    tokens.append(self.SPACE_TOKEN)
            elif c == "\n":
                if self.support_line_break:
                    tokens.append(self.LINE_BREAK_TOKEN)
            else:
                tokens.append(c)
        return tokens
    ...
    def _tok2ids(self, tokens: List[str]) -> list[int]:
        ...
        for s in tokens:
            if s in self._stoi:
                ret.append(self._stoi[s])
            else:
                print(f"ERROR: token not in dict: {s}, unicode: {ord(s)}")
                ret.append(self._stoi[self.UNK_TOKEN])
        return ret
...
class Tokenizer(BaseTokenizer):
    ...
    def __init__(self, charset_path: str) -> None:
        ...
        super().__init__(charset_path, specials_first, specials_last)
    ...
    def encode(
        ...
        batch = [
            torch.as_tensor(
                [self.bos_id] + self._tok2ids(self.tokenize(y)) + [self.eos_id],
                dtype=torch.long,
                device=device,
            )
            for y in labels
...
class CTCTokenizer(BaseTokenizer):
    ...
    def __init__(self, charset_path: str) -> None:
        ...
        super().__init__(charset_path, specials_first=(self.BLANK,))
