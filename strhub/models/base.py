...
from strhub.data.custom_generator import TextLabel, TextLabelBatched
...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def __init__(
        ...
        self.charset_adapter = CharsetAdapter(charset_path)
    ...
    def _eval_step(
        ...
        for pred, prob, gt in zip(preds, probs, labels):
            ...
            ned += edit_distance(pred, gt.text) / max(len(pred), len(gt.text))
            if pred == gt.text:
                correct += 1
            total += 1
...
class CrossEntropySystem(BaseSystem):
    ...
    def __init__(
        ...
        tokenizer = Tokenizer(charset_path)
        super().__init__(
            tokenizer, charset_path, batch_size, lr, warmup_pct, weight_decay
        )
    ...
    def forward_logits_loss(
        ...
        if isinstance(labels, TextLabelBatched):
            labels = labels.text
...
class CTCSystem(BaseSystem):
    ...
    def __init__(
        ...
        tokenizer = CTCTokenizer(charset_path)
        super().__init__(
            tokenizer, charset_path, batch_size, lr, warmup_pct, weight_decay
        )
